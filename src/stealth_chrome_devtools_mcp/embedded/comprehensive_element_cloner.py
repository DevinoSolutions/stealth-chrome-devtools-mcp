    ...
    async def extract_complete_element(
        ...
        try:
            ...
            result = await tab.evaluate(
                js_code, return_by_value=True, await_promise=True
            )
            ...
            if isinstance(result, dict):
                ...
                debug_logger.log_error(
                    "element_cloner", "extract_complete", "Element not found"
                )
                ...
                debug_logger.log_info(
                    "element_cloner",
                    "extract_complete",
                    "Got RemoteObject, extracting value",
                )
                if hasattr(result, "value") and result.value is not None:
                    ...
                    debug_logger.log_error(
                        "element_cloner",
                        "extract_complete",
                        "RemoteObject has no accessible value",
                    )
                    return {
