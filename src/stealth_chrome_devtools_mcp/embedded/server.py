...
async def spawn_browser(
    ...
    try:
        ...
        profile_selection = await clone_storage.resolve_profile_selection(user_data_dir)
        spawn_errors = []
        ...
        for spawn_attempt in range(3):
            ...
            try:
                ...
                if profile_selection.get("profile_role") == "clone":
                    clone_storage._release_clone_dir(selected_user_data_dir)
                fallback_selection = await clone_storage._fallback_profile_selection(
        ...
        if isinstance(spawn_diagnostics, dict):
            spawn_diagnostics["profile_selection"] = (
                clone_storage._public_profile_selection(profile_selection)
            )
            if spawn_errors:
...
async def close_instance(instance_id: str) -> bool:
    ...
    if success:
        ...
        if profile_selection.get("profile_role") == "clone" and profile_selection.get(
            ...
            clone_storage._release_clone_dir(profile_selection["user_data_dir"])
        if should_refresh_snapshot:
            await asyncio.to_thread(
                clone_storage._refresh_master_snapshot_if_safe, "after-master-close"
            )
    return success
