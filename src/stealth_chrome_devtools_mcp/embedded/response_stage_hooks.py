...
class ResponseStageProcessor:
    ...
    async def execute_response_action(
        self, tab, request: RequestInfo, action: HookAction, event
    ) -> None:
        ...
        try:
            ...
            if action.action == "block":
                ...
                debug_logger.log_info(
                    ...
                    f"Blocked response for {request.url}",
                )
                ...
                debug_logger.log_info(
                    "response_stage",
                    "execute_response_action",
                    f"Fulfilled response for {request.url} with custom content",
                )
                ...
                debug_logger.log_info(
                    "response_stage",
                    "execute_response_action",
                    f"Modified response headers/status for {request.url}",
                )
                ...
                debug_logger.log_info(
                    "response_stage",
                    "execute_response_action",
                    f"Continued response normally for {request.url}",
                )
            ...
            debug_logger.log_error(
                "response_stage",
                "execute_response_action",
                f"Error executing response action: {e}",
            )
            ...
            try:
                ...
                debug_logger.log_error(
                    ...
                    "execute_response_action",
                    Exception(
                        f"Failed to continue response after error, response may be stuck: {fallback_err}"
                    ),
                )

    async def execute_request_action(
        self, tab, request: RequestInfo, action: HookAction
    ) -> None:
        ...
        try:
            ...
            if action.action == "block":
                ...
                debug_logger.log_info(
                    ...
                    f"Blocked request {request.url}",
                )

            elif action.action == "redirect":
                await tab.send(
                    uc.cdp.fetch.continue_request(request_id=request_id, url=action.url)
                )
                debug_logger.log_info(
                    "response_stage",
                    "execute_request_action",
                    f"Redirected {request.url} to {action.url}",
                )
                ...
                debug_logger.log_info(
                    "response_stage",
                    "execute_request_action",
                    f"Fulfilled request {request.url}",
                )
            ...
            debug_logger.log_error(
                "response_stage",
                "execute_request_action",
                f"Error executing request action: {e}",
            )
...
response_stage_processor = ResponseStageProcessor(
    None
)  # Will be set by dynamic_hook_system
