...
# THE state dir. settings.py re-derives it (_STATE_DIR_ENV_FILE) because it is a
# leaf module that may not import us; keep both in step, and fork it nowhere else.
STATE_DIR = Path.home() / ".stealth-mcp"
LOCK_FILE = STATE_DIR / "singleton.lock"
PORT_FILE = STATE_DIR / "server.port"
# Records {port, version, pid} so discovery confirms the SAME version before
# reusing the backend we started (issue #14).
SERVER_STATE_FILE = STATE_DIR / "server.json"
DEFAULT_PORT = 19222
# The installed package tree (.../stealth_chrome_devtools_mcp, this file's parent).
# _source_fingerprint() hashes every *.py below it so now-stale source evicts like
# a version mismatch: an editable install's version is frozen (F-206/F-120/F-504).
