...
- **Plans 1–2 of 12 are MERGED to main** (2026-07-07): M3+A1+M10a via #22/#23 (merge commits 37bb34c/3c7b60c), M1 via #24 — main @ bcde11b. **Both Criticals (F-301/F-501) are CLOSED on main**, and each merge was live-validated: 512 unit tests + all 6 gates + 24/24 integration tests green on merged main (integration deliberately re-run after M1 because M1 rewired exactly the liveness paths those tests exercise). The other 10 plans exist only as plans. **Plan 3 (M8+A1, recovery CLI + auto-port-fallback) is in flight** on `audit/fixes-2026-07-02-m8`.
