...
**2.2 A relative request must land STRICTLY INSIDE the clone root.** That is
the seventh refusal in `reserved_reason`. The shared profile is exempt by where
it LANDS rather than by how it was spelled, because `anchor` answers
`roots.shared` for the bare word `default`.
...
`tests/test_profile_anchor_containment.py`, 31 nodes. **RED first: 28 failed,
3 passed** — every dot shape landed on or above a root, both roots were
honoured by absolute path, and `sub/../acme` did not equal `acme`. The 3 that
passed are the guards (an ordinary name, the `sessions/` prefix, an ordinary
absolute path), and they are labelled as such.
...
* **`reserved_reason`'s five existing refusals**, and their order.
