...
| `<input type=date>` | `2024-01-02` | `""` | `""` | no — **this machine only, see below** |
...
**The date row does not generalise, and that was caught by the gate rather than by this table.** It was measured on Chrome 152.0.7977.83, Windows 11, both headed and headless, where the segmented control left its value at `""`. Release-gate run **35039244942** ran the same pin on all three integration cells — Windows/X64, macOS/ARM64, Linux/X64 — and it failed `DID NOT RAISE ToolError` on every one: CI's headless Chrome routes `2024-01-02`'s digits into the date segments, the value changes, and `verify_received` correctly says the page took the input. So whether a date control accepts typed digits is a property of the Chrome build and/or the locale, not of the control. `readonly`, `range` and `color` refused on all four machines (three CI cells + local) and stay pinned as strict refusals; `date` is pinned as the invariant instead — either the tool raised and the value did not move, or it returned and the value did. "Returned `True` over an unchanged value" is what F-873 is, and that is false on both kinds of Chrome.
...
|---|---|
| old fallback — `send_keys("U+E009" + "a")` then `send_keys("U+E017")` | `"<U+E009>a<U+E017>preset-value"` — three characters prepended, the value untouched |
...
* `clear_via_keyboard` — the ONE select-all + Delete. `type_text`'s fallback used to send WebDriver's private-use codepoints (U+E009 for Ctrl, U+E017 for Delete) through `send_keys`, which dispatches them as literal `char` text: CDP has never spoken that protocol, so all **three** characters landed verbatim and nothing was cleared (§2bb). `paste_text` already had a correct CDP version inline; both now call the one function, and the inline copy is deleted.
...
`dom_handler.py` 960 → **904 LOC**; `text_entry.py` is **250**. Both counts are `tools/check_file_budgets.py`'s own rule — `len(read_text().splitlines())`, every line including blanks and comments — because that is the number the gate enforces, and quoting a non-blank count next to it was how "779" got here. No `GRANDFATHER` row is involved: both are under the 1000-LOC default.
...
A raised `ToolError` travels further than the caller, and the field that refused may be a password box. The `section_tool` wrapper hands every escaping exception to `debug_logger.log_tool_failure` (F-835), which records it in the **in-memory ring only** — that entry point deliberately withholds the durable `_backend_logger.error` line, precisely because a tool's failure message echoes the caller's own arguments and F-782 leaves that half open until a redactor exists. The exception itself still goes on to the client, and to Sentry. So the plumbing is: caller + debug ring + Sentry, not the durable log. Every message here therefore reports shape and count only — the selector, how many characters were typed, how many the element holds. Same discipline and the same reason as F-869's storage reader (whose `StorageReadError` *does* reach the durable log, which is why that finding's wording is not this one's).

---

## 5. Verification

* `tests/test_type_text_verification.py` — 14 hermetic pins (9 RED before the fix, the rest positive controls plus the clear and contenteditable pins): the Enter is a real `Input.dispatchKeyEvent`; its `keyDown` carries `text="\r"` + `code` + vk 13; it is dispatched exactly twice (`keyDown`, `keyUp`) and never a third time; `shift_enter` carries modifier 8; each character gets a `keyDown`+`keyUp` pair; a refusing control raises with the selector and the count; the message never carries the typed text; an unreadable read-back raises; a control that accepts still returns `True`; `clear_first`'s baseline is the state AFTER the clear; the clear fallback is Ctrl+A then Delete; a contenteditable is verified through the same path and a refusing one still raises; empty text is not a failure.
* `tests/test_e2e_type_text_verification.py` — 9 real-Chrome pins (6 RED before the fix), own `tmp_empty_root` session root: `#enter-form` (one field, no submit button) is submitted by `parse_newlines`; keydown AND keyup fire, all trusted; `readonly`/`range`/`color` each raise; `date` holds the either/or invariant (raised-and-unmoved, or returned-and-moved) because its answer is build/locale dependent; `number`, the key probe and an i18n string still succeed with the value to prove it.
* `tests/fakes.py` gains `FakeTextField`, a page-backed element double: `FakeTab` routes a key event carrying `text` to the focused field per EVENT, so a double dispatch shows up there exactly as it double-submits in Chrome, and the read-back answer is computed from the double's own state rather than supplied by a test. For the RED commit it also carried nodriver 0.47's `Element.send_keys` reproduced verbatim — the char-only dispatch WAS the thing under test then, and a stub that merely appended the text would have made the pins pass against the very dispatch they exist to reject. That method is **deleted at the fix**: production no longer calls `send_keys`, so nothing reached it, and an unreachable double is dead code whatever its history (the history is in `be95b62`).
* Unit lane at the fix: **2498 passed, 1 skipped** (2484 before this work + the 14 new hermetic pins). FULL integration lane (`-m integration`, real Chrome, 189 nodes): first run **1 failed, 185 passed, 1 skipped, 2 xfailed** — the single failure was the fifth flip in §5a, which the fix was expected to move; confirmation run after flipping it, **186 passed, 1 skipped, 2 xfailed, 0 failed** (962 s).
...
| `test_e2e_interaction_fidelity.py::test_rich_input_types` (range/color arms) | `assert await type_text(...)` then "value unchanged" | `pytest.raises(ToolError)` | same; `number` is unchanged and is the positive control |
| `test_e2e_interaction_fidelity.py::test_rich_input_types` (date arm) | `assert await type_text(...)` then "value unchanged" | the either/or invariant, not either answer | run 35039244942 proved the refusal was local-Chrome-only; a pin true on one machine is a false claim |
...
* **A multi-line `parse_newlines` call whose Enter NAVIGATES reports a failure for a page that accepted it.** The order in `dom_handler.type_text` protects the common `"query\n"` — the trailing line is empty, so nothing is read back after the submit — but it protects nothing beyond that. With `"query\ndetails"` the first Enter submits, the page navigates, and line 2's `entered_text` read hits a node that is no longer attached; that surfaces as `Failed to type text: …`, naming the selector, even though the interaction the caller cared about succeeded. Reading BEFORE the Enter is what makes the empty-line case safe and is the right trade (the alternative — verify after — misreports every successful submit); the residue is this case, and it is a known limit rather than a defect to fix blind. A caller that wants type-then-submit-then-type-again should make it two calls, which is also what the navigation actually is.
* **A control whose value is legitimately IDENTICAL after the interaction now raises.** The inverse of §4a's mask case: a controlled re-render that normalises back to the same string, or a masked field that already held exactly the text being typed, both leave "did anything change" answering no. The tool reports that the page did not accept the input, which in those two shapes is wrong. It is the price of the rule that catches all five measured failures and it is stated here rather than discovered later; the narrower alternative ("does it contain what I typed") is the one §4a rejects, so the trade is between two named false-alarm classes, not between a rule and perfection.
