...
- **LOC-CAP GATE RULING (human, 2026-07-17):** the in-place extraction grows `browser_manager.py` from **1443 → 1532 LOC**, past its prior grandfathered no-grow cap of **1452**. The +89 is irreducible method-boundary overhead (5 signatures/returns/docstrings + orchestrator calls + ruff-mandated separators) — i.e. the modularity/testability gain the budget gate exists to *reward* — so it cannot fit the 9-line headroom, and a manufactured trim would only fight ruff or strip clarity (of C4's 203 added lines, just 16 are blank and essentially all ruff-mandated). Per the C1 precedent (server.py grandfathered at its actual post-format LOC), the repo owner **grandfathered `browser_manager.py` at its actual ruff-clean 1532** (cap == actual, no padding) in `tools/check_file_budgets.py`; the owner string gains `+ plan_M4ph1`. **Caveat vs C1:** no compensating ratchet-down — C1 shrank server.py because code physically *left* the file, whereas C4 is an in-file split, so this is a net increase justified by the seam, not offset by an extraction-out. No-grow applies from this commit forward.
- **ADDENDUM — folded-in robustness fix (human decision, 2026-07-18):** this PR's CI integration lane surfaced a **pre-existing, nodriver-level** flake — nodriver 0.47's `Tab.query_selector` resolves a selector in two *non-atomic* CDP calls (`DOM.getDocument` then `DOM.querySelector(doc.node_id, …)`), so a `DOM.documentUpdated` firing between them invalidates the document nodeId and the second call raises `ProtocolException -32000 "Could not find node with given id"`. It is **not** introduced by M4-Ph1 (the cloner files' only diffs here are STEP 0 import rewrites) but it intermittently failed `test_e2e_data_tools.py` on a *different* select-driven test each run (the race signature). Per the human **fix-first** decision, the fix lands as its own commit folded into this landing: a new leaf `embedded/element_resolution.py` (`resolve_element` / `resolve_by_text` / `query_selector_all`) that recovers from the exact -32000 signal by re-resolving on a **fresh document** — bounded (`_MAX_RESOLVES=3`), other `ProtocolException`s propagate unchanged, a genuinely-absent selector still surfaces as the normal not-found/timeout. The ~11 selector call sites in `dom_handler.py` / `element_cloner.py` / `cdp_element_cloner.py` are routed through it (one resolution path, no second way), pinned hermetically by `tests/test_element_resolution.py` (unit lane 710→717). Behavior-preserving elsewhere.
