...
```bash
uv tool install stealth-chrome-devtools-mcp==2.1.3
```
...
```json
{
  "mcpServers": {
    "stealth-chrome-devtools-mcp": {
      "command": "uvx",
      "args": ["stealth-chrome-devtools-mcp==2.1.3"]
    }
  }
}
```

Or via pip (`pip install stealth-chrome-devtools-mcp==2.1.3`), then use the
