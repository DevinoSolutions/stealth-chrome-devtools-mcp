...
```bash
uv tool install stealth-chrome-devtools-mcp==2.1.8
```
...
```json
{
  "mcpServers": {
    "stealth-chrome-devtools-mcp": {
      "command": "uvx",
      "args": ["stealth-chrome-devtools-mcp==2.1.8"]
    }
  }
}
```

Or via pip (`pip install stealth-chrome-devtools-mcp==2.1.8`), then use the
