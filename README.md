...
```bash
uv tool install stealth-chrome-devtools-mcp==2.1.10
```
...
```json
{
  "mcpServers": {
    "stealth-chrome-devtools-mcp": {
      "command": "uvx",
      "args": ["stealth-chrome-devtools-mcp==2.1.10"]
    }
  }
}
```

Or via pip (`pip install stealth-chrome-devtools-mcp==2.1.10`), then use the
