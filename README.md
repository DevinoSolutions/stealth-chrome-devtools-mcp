...
```bash
uv tool install stealth-chrome-devtools-mcp==2.1.1
```
...
```json
{
  "mcpServers": {
    "stealth-chrome-devtools-mcp": {
      "command": "uvx",
      "args": ["stealth-chrome-devtools-mcp==2.1.1"]
    }
  }
}
```

Or via pip (`pip install stealth-chrome-devtools-mcp==2.1.1`), then use the
