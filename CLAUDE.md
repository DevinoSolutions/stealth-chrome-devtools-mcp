...
| File | Owns |
|---|---|
| `server.py` | the real MCP server — all 94 tool bodies + `app_lifespan` + `mcp`/`registry` + the **binding loop** that drives registration from THIS file's module body, once per execution of it (so the canonical import, the bare-name spec load and the runpy `__main__` load each get a full 94-tool app — a section module that decorated itself would register into the first execution only). The shared substrate the bodies reach for lives in `tool_runtime.py` |
...
**Animations derivation** (five leaves downstream of the engine's animations
...
| `animation_edits.py` | **THE one home for EDIT RECIPES** (M10) — locating a rule's AUTHOR text (`rule_span`, `source_span`, `author_declaration`; never Chrome's `cssText`, which is a re-serialization that matches nothing on disk), the per-knob cascade winner (`winning_rule`: `!important`, then specificity, then document order — degrading when a functional pseudo-class or a computed-value mismatch makes it undecidable), and the token addressing (`token_verdict`, `_swap`) that makes `find`/`replace` mechanically applicable without dropping the rest of the declaration |
