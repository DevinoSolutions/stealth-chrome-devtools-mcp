...
| File | Owns |
|---|---|
| `server.py` | the real MCP server — the **20 tool bodies not yet moved** (only the `browser-management` and `element-interaction` bodies are left; every other section lives in `tool_sections/` as of plan_SERVERSPLIT slices 1-9) + `app_lifespan` + `mcp`/`registry` + the **binding loop** that drives registration from THIS file's module body, once per execution of it (so the canonical import, the bare-name spec load and the runpy `__main__` load each get a full 94-tool app — a section module that decorated itself would register into the first execution only). The shared substrate the bodies reach for lives in `tool_runtime.py` |
