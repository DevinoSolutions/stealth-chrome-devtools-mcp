...
**Animations derivation** (four leaves downstream of the engine's animations
...
| `animation_analysis.py` | **THE one home for the animations schema** — owns `analyze()`, the ONE composition site, plus per-animation records, keyframe resolution, derived timing, direction-aware checkpoints, timeline typing, the transition inventory, and the `CLAIM_FIELDS`/`JUDGEMENT_BLOCKS` registry the confidence invariant test walks |
